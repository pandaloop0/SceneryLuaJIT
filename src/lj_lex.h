...
/* Lua lexer tokens. */
#define TKDEF(_, __) \
  __(and, &&) _(break) _(do) _(else) _(end) _(false) \
  _(for) _(fn) _(goto) _(if) _(in) __(local, let) __(nil, NONE) _(not) __(or, ||) \
  _(repeat) __(return, retfn) _(then) _(true) _(until) _(while) \
